...
use rusty_machine::linalg::{BaseMatrix, Matrix, MatrixSlice};
...
pub fn predict_drag_coefficient(data: &Matrix<f64>) -> f64 {
...
fn get_drag_predictor_neural_network() -> NeuralNet<BCECriterion, StochasticGD> {
    match load_drag_predictor_neural_network() {
        Some(net) => net,
        _ => make_drag_predictor_neural_network()
    }
}

fn load_drag_predictor_neural_network() -> Option<NeuralNet<BCECriterion, StochasticGD>> {
    //TODO: implement this: saving or training and returning a neural net
}

fn save_drag_predictor_neural_network(model: &NeuralNet<BCECriterion, StochasticGD>) {
    let weights: MatrixSlice<f64> = model.get_net_weights(LAYERS.len());
}

fn make_drag_predictor_neural_network() -> NeuralNet<BCECriterion, StochasticGD> {
    ...
    save_drag_predictor_neural_network(&model);
    model
}
